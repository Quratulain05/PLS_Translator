...
export const HomePage = () => {
  ...
  const [mediaRecorder, setMediaRecorder] = useState(null);
  const [recordedChunks, setRecordedChunks] = useState([]);
  const [prediction, setPrediction] = useState(null); // For backend response
  const videoRef = useRef(null);
  const [stream, setStream] = useState(null);
  ...
  const handleRecordToggle = async () => {
    if (!isRecording) {
      ...
      mediaRecorder.onstop = () => {
        const blob = new Blob(recordedChunks, { type: "video/mp4" });
        setSelectedFile(blob); // Set the recorded video as the selected file
        const url = URL.createObjectURL(blob);
        videoRef.current.srcObject = null;
        videoRef.current.src = url;
        videoRef.current.controls = true;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/predict-gesture/", {
        method: "POST",
        body: formData,
      });
      ...
      <div className="flex ml-10 justify-center flex-col" style={{ flex: 4 }}>
        ...
        {prediction && (
          <Typography
            ...
            style={{ fontWeight: "bolder", fontFamily: "Montserrat" }}
          >
            Predicted Gesture: {JSON.stringify(prediction)}
          </Typography>
        )}
      </div>
